    ...
    public class TrianglePrismCellType : ICellType
    {
        ...
        public (CellFaceDir, FaceDetails) RotateBy(CellFaceDir faceDir, FaceDetails faceDetails, CellRotation rotation)
        {
            ...
            else
            {
                ...
                return ((CellFaceDir)FromSide(newSide), newFaceDetails);
            }
        }
        ...
        {
            var ccs = shape.Select(ToTriCoords).ToList();
            var min = ccs.Aggregate(Vector3Int.Min);
            var max = ccs.Aggregate(Vector3Int.Max);
            var a = CoordRotate(rotation, min);
            var b = CoordRotate(rotation, max);
            ...
            {
                var from = cell;
                var to = FromTriCoords(translation + CoordRotate(rotation, cc), cell.y);
